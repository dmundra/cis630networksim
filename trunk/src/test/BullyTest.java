...
public class BullyTest extends AbstractTest {
	...
	private class BullyProcess extends AbstractProcess {
		...
		private BTuple sendElectionP1(Message<ETuple> electionMessage)
			...
			os().logger().info("Election called, start msg is null: " + (electionMessage == null));
			...
			if (electionMessage == null) {
				...
				if (electionMessage.data.electionNum >= electionNum) {
					os().logger().info("my election num: " + electionNum + ", new election num: " + electionMessage.data.electionNum);
					electionNum = electionMessage.data.electionNum;
					
				} else {
					// this means that it's old:
					os().logger().info("this is an old election, ignore election process");
				...
				while (meLead) {
					...
					if (message.data.electionNum > electionNum) {
						...
						if (message.data.address > os().address()) {
							...
							os().logger().info(
									"responding to a inferior chaps request to vote: "
											+ message.data.address);
						} else {
							...
							os().logger().info(
									"someone higher than I responded: "
											+ message.data.address);
						}
					}
		...
		protected void run() throws InterruptedException {
			...
			while (true) {
				...
				try {
					...
					//clear off our buffer until we see that we need an election:
					while(!callElection){
...
//					}
				...
				if (callElection || !sendToLeader()) {
					os().logger().info("Election code from run() electionMessage is null: " + (electionMessage == null));
...
//		members.add(5);
		...
		final Node a = sim.buildNode(1).name("A").kernel(
				sim.createUserKernel(new BullyProcess((ArrayList<Integer>)members.clone(), 0))).create();
...
//				sim.createUserKernel(new BullyProcess((ArrayList<Integer>)members.clone(), 0))).create();
		...
		final Node router1 = sim.buildNode(6).name("Router1").connections(a, b).create();
