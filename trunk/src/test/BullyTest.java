...
public class BullyTest extends AbstractTest {
	...
	private class BullyProcess extends AbstractProcess {
			...
			if (os().address() != leader) {
				...
				try {
					Message<String> message = os().receive(WORK_PORT,
					        WAIT_FOR_LEADER_RESPONSE, TimeUnit.MILLISECONDS)
					        .asType(String.class);
					os().logger()
					        .log(Level.INFO,
		 ...
		 * 
		 * @param newElection
		 *            if true, we'll increment our election clock and send our
		 *            messages. If false, we'll send our messages only if we've
		 *            never done so (for this election)
		 * @throws InterruptedException
		 */
		private void callElection(boolean newElection)
			...
			if (electionNumLastRequested < electionNum) {
				...
				throw new RuntimeException(
				        "error in our logic, we are calling a new election when the last requested election number is not less than the election number");
			}
		 ...
		 * 
		 * @param timeOut
		 *            initial time out we should use when popping messages
		 * @throws InterruptedException
		 */
		 ...
		 * @return true if we think that a victory message should be sent (if we
		 *         are the winner)
		 * @throws InterruptedException
		 */
		private boolean sendElectionP1(Message<ETuple> electionMessage)
			...
			if (electionMessage.data.electionNum > electionNum) {
				os().logger().info(
				        "updating my election num: " + electionNum
				                + ", new election num: "
			...
			if (electionMessage.data.address < os().address()) {
				...
				try {
					...
					os().logger().info(
					        "responding to an inferior - "
					                + electionMessage.source);
		 ...
		 * 
		 * @return false if we were expecting a victory message, but never got
		 *         it, true otherwise.
		 * @throws InterruptedException
		 */
		 ...
		 * handles our phase 2 (sending victory or receiving victory message)
		 * 
		 * @param resendWinningMessage
		 *            if true, we'll resend the victory message if we were the
		 *            winner from the current election
		 * @return false if we were expecting a victory message, but never got
		 *         it, true otherwise.
		 * @throws InterruptedException
		 */
		public boolean electionP2(boolean resendWinningMessage)
		        throws InterruptedException {

			if (resendWinningMessage && elecitonNumLastReceived == electionNum
			        && iShouldBeLeader && leader == os().address()) {

				// we need to resend our victory (this will be reupdated in the
				// next block):
			...
			if (elecitonNumLastReceived < electionNum) {
					...
					try {
						...
						while (true) {
							...
							if (message.data.electionNum < electionNum) {
								...
								throw new RuntimeException(
								        "error in our logic: we say it's election #"
								                + electionNum + ", but got a "
								                + message.data.electionNum);
							}
						}
				...
				try {
					Message<ETuple> message = os().receive(RESULT_PORT, 1,
					        TimeUnit.MILLISECONDS).asType(ETuple.class);
					...
					if (message.data.address < leader
					        && os().address() == leader) {
						// this is from someone that could be our new leader
						// and we think that we are the leader, let's call an
						// election:
						callElection(true);
						return true;
					}
		 ...
		 * 
		 * @param electionNum
		 *            the new election number
		 */
		private void setElectionNum(int electionNum) {

			if (electionNum <= this.electionNum) {
				throw new RuntimeException(
				        "error in our logic, we are trying to set our election to something less than or equal to the current");
			}
	 ...
	 * 
	 * Each process has a logical election clock. This clock represents the most
	 * current election that the process knows about. When a process calls an
	 * election, it increments the clock and begins sending the messages. When
	 * an election related message is received, the election clock is checked to
	 * see if the message is relevant (i.e. not from an old election). The use
	 * of the election clocks enabled us to have multiple processes calling
	 * elections concurrently while still arriving at a single result. For
	 * example:
	 * 
	 * When we start the test, the leader is a dead process, so all process call
	 * an election after incrementing their clocks to 1 (the clock starts at 0).
	 * So as far as each process is concerned, they started the election, but
	 * their action isn't any different than if they received a request for
	 * election. So the first election ends with a leader (except in the case
	 * that the leader hits the high end of a random sleep, in which case, the
	 * 'real' leader calls another election). So b eventually becomes the leader
	 * (after one or two elections).
	 * 
	 * a then joins the network. a calls an election and declares itself the
	 * winner (of election 1). b (the current master, and winner of election 1)
	 * sees this, and calls a new election, in which a is again declared winner
	 * (this time of election 2) and b becomes a regular slave.
	 * 
	 * After some time, a dies, and a new election is called, to which b wins.
	 * After another interval, a new process joins. This process calls an
	 * election (with election time 1), to which processes respond (and thus the
	 * clock is updated to the current election, #3). When b receives the
	 * message to cast its vote, it also sees that it has already declared
	 * itself the winner to the current election, so it resends the victory
	 * message.
	 * 
	...
	public void test5Nodes() throws InterruptedException {
		...
		final Node router1 = sim.buildNode(startIndex + 6).name("Router1")
		        .connections(a, b, c).create();
		@SuppressWarnings("unused")
		final Node router2 = sim.buildNode(startIndex + 7).name("Router2")
		        .connections(router1, d, e).create();

		sim.start();

		Thread.sleep(40000);
		//kill a
		sim.logger().info("______________killing leader______________");
	...
	/**
	 * this class is the data we pass around in election messages
	 */
